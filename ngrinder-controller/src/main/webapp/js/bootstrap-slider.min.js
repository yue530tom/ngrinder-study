...
this.step.toString().indexOf(".");if(~d)this.decimals=this.step.toString().length-(d+1);this.slider=c(this.options.template).appendTo(this.options.slider).get(0);this.disabled(this.element.disabled);c(this.element).bind({"change.slider":c.proxy(x,this),"keydown.slider":c.proxy(y,this),"mousewheel.slider":c.proxy(l,this),"DOMMouseScroll.slider":c.proxy(l,this)});c(this.slider).bind({"mousedown.slider":c.proxy(z,this),"mousewheel.slider":c.proxy(l,this),"DOMMouseScroll.slider":c.proxy(l,this)});if("step"in
this.element)this.element.step=this.step;u.call(this);v.call(this,{});f.call(this,m(Math.max(Math.min(e,this.max),this.min),this.step))}function u(){var a=c(this.slider),b=c(this.slider.firstChild),e=a.width(),d=a.height();this.box=a.offset();this.knobBox={width:b.width(),height:b.height()};this.box.right=e+this.box.left;this.box.bottom=d+this.box.top;this.box.isHorizontal=e>=d}function v(a){var b;a.target===this.slider.firstChild?(b=c(this.slider.firstChild).offset(),this.dragOffset=this.box.isHorizontal?
a.pageX-b.left:this.knobBox.height-(a.pageY-b.top)):this.dragOffset=this.box.isHorizontal?this.knobBox.width/2:this.knobBox.height/2}function x(a){var b=j(this.element.value,this.min);b===this.min&&"input"==a.type||f.call(this,m(Math.max(Math.min(b,this.max),this.min),this.step))}function y(a){var b=j(this.element.value,this.min);if(!this.isDisabled)switch(a.which){case 38:a.preventDefault();f.call(this,Math.min(this.max,b+this.step));c(this.element).trigger("slide");break;case 40:a.preventDefault();
...
a.which&&!this.isDisabled){var b=c.Event("slidestart");c(this.element).trigger(b);b.isDefaultPrevented()?a.preventDefault():(u.call(this),v.call(this,a),A(),c(i).bind({"mousemove.slider":c.proxy(w,this),"mouseup.slider":c.proxy(n,this),"selectstart.slider":t,"dragstart.slider":t}),c(this.slider).addClass("focused"),this.isSliding=!0,w.call(this,a))}}function n(){if(this.isSliding)c(i).unbind(".slider"),c(this.slider).removeClass("focused"),this.isSliding=!1,c(this.element).trigger("slideend")}function w(a){if(1!==
